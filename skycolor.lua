...
local function get_sky_color (pos)
	return mcl_weather.get_sky_color(pos)
end

local function get_fog_color (pos)
	return mcl_weather.get_fog_color (pos)
end

local function get_both_colors (pos)
	return mcl_weather.get_sky_fog_colors (pos)
end
...
mcl_weather.skycolor = {
	...
	update_sky_color = function(players)
		...
		for _, player in ipairs(players) do
			local pos = player:get_pos()
			local has_weather = (mcl_weather.state =="rain" or mcl_weather.state == "thunder") and mcl_weather.has_snow(pos) or ((mcl_weather.state =="rain" or mcl_weather.state == "thunder") and mcl_weather.has_rain(pos))
...
local function initsky(player)
	...
	player:set_clouds({height=127, speed={x=-2, z=0}, thickness=4, color="#FFF0FEF"})
end
